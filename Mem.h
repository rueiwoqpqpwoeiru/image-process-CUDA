...
template<typename T> struct D_data {
	...
	unsigned int ch;
	// read & write
	__host__ __device__ __forceinline__ T& val(unsigned int w_, unsigned int h_, unsigned int ch_) const { return ((T*)((char*)(ptr) + pitch * (h * ch_ + h_)))[w_]; }
	// read only (Faster than val(), because no need to multiply pitch)
	__device__ __forceinline__ T tex(float w_, float h_, unsigned int ch_) const { return tex2D<T>(t, (w_), (h * ch_ + h_)); }
};
...
template<typename T> struct D_mem {
	...
	void use_tex(const cudaTextureFilterMode& interpolation = cudaFilterModePoint) {
		// Set boundary conditions, subpixel interpolation method, normalize coordinates, etc.
		cudaTextureDesc texDesc;
		memset(&texDesc, 0, sizeof(texDesc));
		texDesc.addressMode[0] = cudaAddressModeWrap;
		texDesc.addressMode[1] = cudaAddressModeWrap;
		texDesc.filterMode = interpolation;
		...
		Stopwatch sw;
		cudaCreateTextureObject(&d_data.t, &resDesc, &texDesc, nullptr);
		sw.print_time(__FUNCTION__);
